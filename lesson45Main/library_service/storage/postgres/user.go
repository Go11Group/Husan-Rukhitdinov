...
func (u *UserRepo) UpdateUser(user *pb.UpdatedUserRequest) (*pb.Void, error) {
	_, err := u.Db.Exec("update user set name = $1,surname = $2,updated_at = $3 where id = $4 and deleted_at = 0", user.Name, user.LasName, time.Now(), user.Id)
	if err != nil {
		return nil, err
	}
	return nil, nil
...
func (u *UserRepo) DeleteUser(id *pb.ByIdRequest) (*pb.Void, error) {
...
func (u *UserRepo) GetUser(f *pb.UserFilterRequest) (*pb.UsersResponse, error) {
	var (
